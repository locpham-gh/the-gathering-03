...
| 2.0     | 2026-02-23 | Rewritten to match current The Gathering project |
| 2.1     | 2026-02-28 | Added Whiteboard, Admin Panel, and Enhanced Maps |
| 2.2     | 2026-03-28 | Added Spatial Chat, Phone Sync, and Role-based Dashboard |
| 2.3     | 2026-03-30 | Implemented Scalable Admin (Pagination), Message De-duplication, and Improved Sync |
| 2.4     | 2026-04-20 | Implemented Directional Seat Snapping, Web Audio API Spatial Audio, and Glassmorphic UI |
...
### 2.7 Advanced Game Features

- **Directional Seat Snapping**: Players can snap to chairs using the 'E' key. The system automatically aligns the player's sprite direction (up, down, left, right) based on the chair's orientation defined in the map config.
- **Web Audio API Spatial Audio**: Replaced standard audio with a custom Web Audio API implementation featuring exponential volume decay and stereo panning. This ensures that audio sources feel physically located in the 2D space relative to the listener.
- **Spatial Video Overlay**: Implemented a fixed-position video overlay that renders remote participants' video tracks directly above their avatars on the canvas, converting world coordinates to screen coordinates in real-time.
- **Spatial Chat (Nearby Chat)**: Integrated location-based message filtering. The client only displays messages from players within a 250px radius, simulating real-life proximity conversations.
- **Phone Interaction (Q Key)**: Two-stage animation (Pull out & Keep) synchronized instantly via WebSocket. This signal bypasses the standard movement throttle for immediate broadcast.
- **Atomic Room Joining**: Uses MongoDB's `$addToSet` for joining rooms to prevent version conflicts and ensure data integrity during simultaneous joins or snapshots.
- **Theme Persistence**: Support for Light/Dark mode with user preference remembered via LocalStorage and applied across all UI components (Sidebar, Chat, Modals).
- **Fullscreen Immersive Views**: Chat and Calendar modules use a fullscreen overlay architecture with their own sidebar systems to maximize interaction space.
- **Image-Based Backgrounds**: Supports high-quality static images (PNG/JPG) as map backgrounds instead of traditional tilesets, optimizing performance while achieving maximum aesthetics.
- **Collaborative Whiteboard**: Integrated Excalidraw with real-time WebSocket synchronization and MongoDB persistence.
- **Glassmorphic UI**: The bottom control bar and various overlays use a glassmorphic aesthetic (backdrop-filter, subtle borders, high transparency) to provide a premium, modern feel.
- **Map Normalization**: Standardized map ID routing for "classroom", "office_combined", and "cafe" to ensure consistent asset loading across client and server.
- **Reliable Persistence**: Periodic snapshots every 30 seconds ensure player positions are preserved even during server restarts, replacing the need for Redis.
- **Multi-modal Auth**: Integration of Google One Tap for seamless entry and Email OTP for secure alternative login.
