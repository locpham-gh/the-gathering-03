...
usecaseDiagram
    ...
    package "The Gathering System" {
        usecase "Register Account" as UC1A
        usecase "Login Account" as UC1B
        usecase "Join Room" as UC5
        usecase "Real-time Position Sync" as UC8
        usecase "Update Profile" as UC3
        usecase "Create Room" as UC4
        usecase "Collaborative Whiteboard" as UC18
        usecase "Directional Seat Snapping" as UC24
        usecase "Proximity Call" as UC09
        usecase "Community Forum" as UC12
        usecase "Admin Management" as UC19
        usecase "Schedule & Manage Events" as UC10
        usecase "Digital Library Access" as UC14
        usecase "Spatial Video & Audio" as UC23
        usecase "System Integrity & Persistence" as UC21
    }
...
rectangle "Authentication & Profile Module" {
    ...
    (Login Account) as UC1B
    (Update Profile) as UC3
    (Toggle Theme) as UC16
    (External Authentication) as UC_Ext
    (Verification Code) as UC_Code
}
...
UC1A ..> UC_Ext : <<include>>
UC1A ..> UC_Code : <<include>>
UC1B ..> UC_Ext : <<include>>
UC1B ..> UC_Code : <<include>>
@enduml
```
...
##### _d) Use case Toggle Theme_

**Use Case ID:** UC_16

**Use Case Name:** Toggle Theme (Light/Dark Mode)

**Brief Description:** The actor switches between Light and Dark visual themes to suit their preference or environmental lighting.

**Actor:** Authenticated User

**Pre-conditions:** The user is logged in.

**Post-conditions:** The system theme is updated and the choice is persisted in local storage or the user's profile.

**Main Success Flow:**
1. The actor clicks the "Theme Toggle" (Sun/Moon) icon on the dashboard or game sidebar.
2. The system detects the current active theme.
3. The system applies the alternative CSS color variables (Dark -> Light or vice versa).
4. The system saves the preference to the database.
5. The system confirms the change with a subtle UI transition.
...
Table 24: Use case Description - Toggle Theme
...
rectangle "Room Management Module" {
    (Create Room) as UC4
    (Join Room) as UC5
    (Manage Members) as UC6
    (Generate Invite Code) as UC_Invite
}

AU --> UC4
AU --> UC5
RO --> UC6
UC4 ..> UC_Invite : <<include>>
@enduml
```
...
usecaseDiagram
    ...
    package "Room Management Module" {
        usecase "Create Room" as UC4
        usecase "Join Room" as UC5
        usecase "Manage Members" as UC6
        usecase "Generate Invite Code" as UC_Invite
    }
...
##### _c) Use case Manage Room Members_

**Use Case ID:** UC_06

**Use Case Name:** Manage Room Members

**Brief Description:** The Room Owner moderates the workspace by renaming the room or removing problematic participants.

**Actor:** Room Owner

**Pre-conditions:** The user is logged in and is the designated owner of the active room.

**Post-conditions:** Room settings are updated or a member is disconnected and removed from the membership list.

**Main Success Flow:**
1. The actor opens the "Room Settings" modal from the sidebar.
2. The system displays a list of all current room members.
3. The actor clicks the "Kick" button next to a specific member's name.
4. The system prompts for confirmation.
5. The actor confirms the action.
6. The system removes the member from the database and terminates their active WebSocket connection.
7. The system updates the member list for all remaining participants.

**Alternative Flows:**
A1: Rename Room
1. In step 2, the actor enters a new name in the "Room Name" field.
2. The actor clicks "Save Changes".
3. The system updates the room metadata in the database.

**Exception Flows:**
E1: Unauthorized Action
In step 1 of the main flow: If the user is not the owner, the system hides the management options or returns a "Forbidden" error.

Table 20: Use case Description - Manage Room Members
...
rectangle "Spatial Interaction Module" {
    (Real-time Position Sync) as UC8
    (Directional Seat Snapping) as UC24
    (Proximity Audio/Video Call) as UC09
    (Spatial Media Experience) as UC23
    (Phone Interaction) as UC15
    (View Mini-map) as UC20
...
UC09 ..> UC_Detect : <<include>>
UC23 ..> UC_Detect : <<include>>
UC24 --|> UC8 : <<extend>>
@enduml
```
...
usecaseDiagram
    ...
    package "Spatial Interaction Module" {
        usecase "Real-time Position Sync" as UC8
        usecase "Directional Seat Snapping" as UC24
        usecase "Proximity Audio/Video Call" as UC09
        usecase "Spatial Media Experience" as UC23
        usecase "Phone Interaction" as UC15
        usecase "View Mini-map" as UC20
        usecase "Detect Neighbors" as UC_Detect
    }
...
##### _a) Use case Real-time Position Sync_
...
##### _b) Use case Directional Seat Snapping_
...
##### _c) Use case Proximity Audio/Video Call_
...
##### _d) Use case Spatial Media Experience_
...
##### _e) Use case Phone Interaction_

**Use Case ID:** UC_15

**Use Case Name:** Phone Interaction (Q Key)

**Brief Description:** The actor toggles a mobile phone animation to signal their "away" or "chatting" status to other participants visually.

**Actor:** Authenticated User

**Pre-conditions:** The user is inside a virtual room and their avatar is rendered on the canvas.

**Post-conditions:** The character sprite is updated with a phone-holding animation frame.

**Main Success Flow:**
1. The actor presses the 'Q' key on their keyboard.
2. The system checks the current "isUsingPhone" state of the character.
3. The system toggles the state (True/False).
4. The system updates the local character animation.
5. The system broadcasts the state change via WebSocket to all other participants.
6. Other participants' clients render the phone sprite for the actor's avatar.
...
Table 21: Use case Description - Phone Interaction

##### _f) Use case Mini-map & Environment_

**Use Case ID:** UC_20

**Use Case Name:** Mini-map & Environment

**Brief Description:** The actor uses a top-down mini-map overlay to navigate large virtual workspaces and locate other participants quickly.

**Actor:** User

**Pre-conditions:** The user is inside a virtual room.

**Post-conditions:** A real-time navigation overlay is displayed on the screen.

**Main Success Flow:**
1. The actor clicks the "Mini-map" icon on the game HUD.
2. The system generates a scaled-down representation of the current map.
3. The system overlays icons representing the actor and other active participants on the map.
4. The system updates the icons' positions in real-time based on movement data.
5. The actor clicks the icon again to close the overlay.
...
Table 22: Use case Description - Mini-map
...
rectangle "Collaboration Module" {
    ...
    (Digital Library Access) as UC14
    (Schedule & Manage Events) as UC10
    (Fullscreen Overlays) as UC17
    (Send Email Invitation) as UC_Email
}
...
usecaseDiagram
    ...
    package "Collaboration Module" {
        ...
        usecase "Digital Library Access" as UC14
        usecase "Schedule & Manage Events" as UC10
        usecase "Fullscreen Overlays" as UC17
        usecase "Send Email Invitation" as UC_Email
    }
...
##### _c) Use case Digital Library Access_
...
##### _e) Use case Fullscreen Overlays_

**Use Case ID:** UC_17

**Use Case Name:** Fullscreen Overlays

**Brief Description:** The actor switches to an immersive, distraction-free view for specific tools like the Calendar or Forum for a better focus.

**Actor:** User

**Pre-conditions:** The user is logged in and accessing the room features.

**Post-conditions:** The UI transitions from a sidebar/component view to a centered fullscreen overlay.

**Main Success Flow:**
1. The actor selects the "Expand" icon on a feature panel (e.g., Calendar).
2. The system applies a glassmorphic blur to the background game canvas.
3. The system renders the feature component as a high-resolution, centered overlay.
4. The actor interacts with the feature (e.g., browsing events).
5. The actor clicks the "Close" or "Escape" button.
6. The system restores the standard room view.
...
Table 23: Use case Description - Fullscreen Overlays
...
rectangle "Administration Module" {
    ...
    (Backup Room State) as UC_Backup
}

AD --> UC19
SY --> UC21

UC21 ..> UC_Monitor : <<include>>
UC21 ..> UC_Backup : <<include>>
@enduml
```
...
usecaseDiagram
    ...
    package "Administration Module" {
        usecase "Admin Management" as UC19
        usecase "System Integrity & Persistence" as UC21
        usecase "Monitor Traffic" as UC_Monitor
        usecase "Backup Room State" as UC_Backup
    }
...
##### _b) Use case System Integrity & Persistence_
