            ...
            <div className="space-y-3 animate-in fade-in slide-in-from-top-4 duration-300 relative z-10 px-2 py-4">
              {members.map((member) => {
                ...
                return (
                  <div
                    key={member._id}
                    className={`flex items-center gap-3 p-3 rounded-2xl transition-all ${isOnline ? "bg-white shadow-sm border border-slate-200" : "opacity-60 grayscale-[50%]"}`}
                  >
                    ...
                    <div className="flex-1 overflow-hidden">
                      <p
                        className={`font-bold truncate text-sm ${isOnline ? "text-slate-800" : "text-slate-500"}`}
                      >
                        {member.displayName}
                      </p>
