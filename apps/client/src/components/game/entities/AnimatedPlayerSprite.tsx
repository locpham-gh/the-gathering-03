...
export const AnimatedPlayerSprite: React.FC<AnimatedPlayerSpriteProps> = ({
  ...
  useEffect(() => {
    if (emote) {
      Promise.resolve().then(() => setEmoteState({ id: emote.id, yOffset: -80, alpha: 1 }));
    }
  }, [emote]);

  useEffect(() => {
    if (chatBubble) {
      const text = chatBubble.length > 28 ? chatBubble.slice(0, 28) + "…" : chatBubble;
      Promise.resolve().then(() => setBubbleState({ text, alpha: 1 }));
    }
  }, [chatBubble]);
  ...
  let baseCol = DIR_COL_OFFSET[direction] || 0;
  ...
  if (isSitting) {
    if (direction === "left" || direction === "right") {
      ...
      row = 1;
      baseCol = DIR_COL_OFFSET[direction] || 0;
