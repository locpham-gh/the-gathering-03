...
import { Video, Keyboard, Trash2, ExternalLink, Shield, Users } from "lucide-react";
      ...
      {isRoomsView ? (
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            <h3 className="text-2xl font-bold text-slate-800 tracking-tight">Your Workspaces</h3>
            <span className="text-sm text-slate-400 font-medium bg-slate-100 px-3 py-1 rounded-full">{rooms.length} Rooms</span>
          </div>

          <div className="grid md:grid-cols-2 gap-6">
            {rooms.length === 0 && !loading && (
              <div className="col-span-2 py-20 text-center bg-white border border-dashed border-slate-200 rounded-3xl">
                <p className="text-slate-400">You haven't created or joined any rooms yet.</p>
              </div>
            )}
            ...
            {rooms.map((room) => {
              ...
              return (
                <div key={room._id} className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 hover:shadow-md transition-all group relative overflow-hidden">
                  {isOwner && (
                  ...
                  <div className="mb-4">
                     <h4 className="text-lg font-bold text-slate-800 mb-1">{room.name}</h4>
        ...
        <div className="space-y-12 animate-in fade-in slide-in-from-bottom duration-700">
          <div className="space-y-2">
            ...
            <p className="text-slate-500 font-medium">What's on your agenda today?</p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 hover:shadow-xl hover:border-primary/20 transition-all group">
              ...
              <h3 className="text-xl font-bold bg-white mb-2">Create New Meeting</h3>
              <p className="text-slate-500 text-sm mb-6 leading-relaxed">Start an instant session with a professional virtual room code.</p>
            ...
            <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 hover:shadow-xl hover:border-slate-300 transition-all group">
              ...
              <h3 className="text-xl font-bold text-slate-800 mb-2">Join with Code</h3>
              <p className="text-slate-500 text-sm mb-6 leading-relaxed">Have a code from a colleague? Enter it below to jump in.</p>
              ...
              <form onSubmit={handleJoin} className="flex gap-2">
                ...
                />
                <button type="submit" disabled={!roomCode.trim()} className="px-6 bg-slate-800 text-white py-3 rounded-xl font-bold hover:bg-slate-900 transition-all shadow-lg shadow-slate-200 disabled:opacity-50">
                  Join
                </button>
              </form>
            </div>
...
function ProfileSettings({ user }: { user: any }) {
  ...
  return (
    <div className="max-w-xl animate-in fade-in slide-in-from-right duration-500">
      <h3 className="text-2xl font-bold text-slate-800 mb-8">Personal Information</h3>
      <form onSubmit={handleUpdate} className="space-y-6">
        ...
        <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center gap-4">
           <img src={avatar} className="w-16 h-16 rounded-full border border-white shadow-xl" alt="Preview" />
           <p className="text-xs text-slate-400 leading-relaxed italic">Changes will reflect across all your virtual rooms and activity.</p>
        </div>
        <button 
          type="submit" 
          disabled={saving}
